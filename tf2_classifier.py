...
class Classifier(object):
    ...
    def _build_model(self, args):
        ...
        self.checkpoint = tf.train.Checkpoint(
            classifier_optimizer=self.classifier_optimizer, classifier=self.classifier
        )
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(batch_idx):
                ...
                batch_label = (
                    ...
                    .astype(np.float32)
                    .reshape(len(batch_label), 2)
                )

                with tf.GradientTape(persistent=True) as tape:

                    # Origin samples passed through the classifier
                    origin = self.classifier(batch_data, training=True)
                    test = self.classifier(data_test, training=True)
                    ...
                    test_prediction = tf.equal(
                        tf.argmax(test_softmax, 1), tf.argmax(label_test, 1)
                    )
                    test_accuracy = tf.reduce_mean(tf.cast(test_prediction, tf.float32))

                # calculate gradients
                classifier_gradients = tape.gradient(
                    target=loss, sources=self.classifier.trainable_variables
                )

                # apply gradients to the optimizer
                self.classifier_optimizer.apply_gradients(
                    zip(classifier_gradients, self.classifier.trainable_variables)
                )
                ...
                if idx % 100 == 0:
                    ...
                    print(
                        (
                            "Epoch: [%2d] [%4d/%4d] loss: %6.2f, accuracy: %6.2f"
            ...
            print(
                (
                    "Epoch: [%2d] loss: %6.2f, accuracy: %6.2f"
    ...
    def test(self, args):
        ...
        for idx in range(len(sample_files)):
            ...
            origin_softmax = tf.nn.softmax(
                self.classifier(origin * 2.0 - 1.0, training=False)
            )
            transfer_softmax = tf.nn.softmax(
                self.classifier(transfer * 2.0 - 1.0, training=False)
            )
            cycle_softmax = tf.nn.softmax(
                self.classifier(cycle * 2.0 - 1.0, training=False)
            )
